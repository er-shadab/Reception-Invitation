...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setAll = (cls, txt) =>
    document.querySelectorAll(cls).forEach((e) => (e.textContent = txt));
  ...
  let EVENT_DATE = new Date("2026-10-28T19:00:00");
  ...
  (function applyParams() {
    ...
    if (n1 || n2) {
      ...
      document.title =
        (n1 || "Shadab") + " & " + (n2 || "Ronak") + " — Reception Party";
    }
    ...
    if (addr || city)
      setAll(".js-addr", [addr, city].filter(Boolean).join(", "));
    if (venue || city) {
      ...
      if (mapEmbed)
        mapEmbed.src =
          "https://maps.google.com/maps?q=" + queryStr + "&z=13&output=embed";
  ...
  const NOTES = [293.66, 311.13, 369.99, 392.0, 440.0, 466.16, 554.37]; // Hijaz oriental scale
  ...
  if (env) {
    ...
    ["pointerup", "pointercancel", "pointerleave"].forEach((ev) =>
      env.addEventListener(ev, onHoldEnd),
    );
  ...
  (function initFloaters() {
    ...
    const shapes = [
      "<svg width='9' height='9' viewBox='0 0 9 9'><circle cx='4.5' cy='4.5' r='3.8' fill='#F6D896' opacity='.9'/><circle cx='3.2' cy='3.2' r='1.2' fill='#FFFFFF' opacity='.95'/></svg>",
      "<svg width='7' height='7' viewBox='0 0 7 7'><circle cx='3.5' cy='3.5' r='2.8' fill='#E5B34C' opacity='.85'/></svg>",
      "<svg width='11' height='11' viewBox='0 0 11 11'><circle cx='5.5' cy='5.5' r='4.8' fill='#FDF4DE' opacity='.8'/><circle cx='4' cy='4' r='1.5' fill='#FFFFFF' opacity='.95'/></svg>",
    ];
  ...
  function setupScratchCards() {
    ...
    scratchCards.forEach((cv) => {
      ...
      gradient.addColorStop(0, "#F5DE9E");
      gradient.addColorStop(0.5, "#C89938");
      gradient.addColorStop(1, "#E6BE5C");
  ...
  if (rsvpBtn) {
    rsvpBtn.addEventListener("click", () => {
      ...
      const groom =
        (document.querySelector(".js-n1") &&
          document.querySelector(".js-n1").textContent) ||
        "Shadab";
      const bride =
        (document.querySelector(".js-n2") &&
          document.querySelector(".js-n2").textContent) ||
        "Ronak";
      ...
      const waUrl =
        "https://wa.me/" + RSVP_PHONE + "?text=" + encodeURIComponent(message);
